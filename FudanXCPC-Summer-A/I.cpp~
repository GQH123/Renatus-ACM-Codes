...
#define maxn 320 
#define assert(false) printf("%d", 1 / 0)
using namespace std;
...
int main(){ 
    ...
    while (T--){
        ...
        rep(line, 1, n){
            ...
            while (1){    
                int eof;
                while ((eof = scanf("%c", &ch), eof == 1) && (ch == ' ' || ch == '\t' || ch == '\r'));
                if (ch == '\n' || eof == EOF) {if (!over) over = process(cs); break;}
                if (ch == '"') {
                    int l = 0;
                    sta[cs][l++] = ch;
                    while ((eof = scanf("%c", &ch), eof == 1) && ch != '"') sta[cs][l++] = ch;
                    sta[cs][l++] = ch;
                    sta[cs][l] = '\0';
                    cs++;
                }
                    ...
                    int l = 0;
                    sta[cs][l++] = ch;
                    while ((eof = scanf("%c", &ch), eof == 1) && ch != ' ' && ch != '\n' && ch != '\t' && ch != '=' && ch != '\r') sta[cs][l++] = ch;
                    sta[cs][l] = '\0';
                    cs++;
                    if (ch == '=') {
                        l = 0;
