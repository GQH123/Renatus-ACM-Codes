...
void solve(int x) {
	...
	int ptr = cct;
	while (deg[x] && ptr >= 1) {
		ans += tmp[ptr].fi - v[find_root(x)];
		merge(x, tmp[ptr].se); 
		deg[tmp[ptr].se]--, deg[x]--, ptr--;
	}
}
...
void work(){
	...
	per(i, n, 1) solve(lis[i].se);
	
	rep(i, 1, n) if (find_root(i) == i) ans += v[i];
	
	rep(i, 1, n) cerr << find_root(i) << ' '; cerr << endl;
