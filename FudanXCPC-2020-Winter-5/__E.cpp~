...
int DP(int pos, int rst, bool opf, bool opm){
	...
	ans = inf;
	if (rst) ans = min(ans, DP(pos + 1, max(0, rst - m[pos]), opf | 1, opm | 1) + max(m[pos], 2 * m[pos] - rst));
	else ans = min(ans, DP(pos + 1, rst, opf | 1, opm) + 2 * m[pos]);
