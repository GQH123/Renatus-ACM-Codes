...
ll dp2[maxn][maxn][maxn][maxn];
int vis2[maxn][maxn][maxn][maxn]; 
ll DP_S(int pos, int n1, int n2, int n3) {
	...
	ll res1, res2;
	if (n == 3) {
		res1 = DP_S(pos + 1, n1 - (ans[1][pos] == 'T'), n2 - (ans[2][pos] == 'T'), n3 - (ans[3][pos] == 'T'));
		res2 = DP_S(pos + 1, n1 - (ans[1][pos] == 'F'), n2 - (ans[2][pos] == 'F'), n3 - (ans[3][pos] == 'F')); 
	}
	else if (n == 2) {
		res1 = DP_S(pos + 1, n1 - (ans[1][pos] == 'T'), n2 - (ans[2][pos] == 'T'), 0);
		res2 = DP_S(pos + 1, n1 - (ans[1][pos] == 'F'), n2 - (ans[2][pos] == 'F'), 0); 
	}
	else {
		res1 = DP_S(pos + 1, n1 - (ans[1][pos] == 'T'), 0, 0);
		res2 = DP_S(pos + 1, n1 - (ans[1][pos] == 'F'), 0, 0); 
	}
	 
	return anss = res1 + res2;
}

ll DP_P(int pos, int n1, int n2, int n3) {
	if (n1 < 0 || n2 < 0 || n3 < 0) return 0;
	if (pos == 0) {
		return !n1 && !n2 && !n3;
	} 
	...
	ll& anss = dp2[pos][n1][n2][n3]; anss = 0;
	ll res1, res2;
	if (n == 3) {
		res1 = DP_P(pos - 1, n1 - (ans[1][pos] == 'T'), n2 - (ans[2][pos] == 'T'), n3 - (ans[3][pos] == 'T'));
		res2 = DP_P(pos - 1, n1 - (ans[1][pos] == 'F'), n2 - (ans[2][pos] == 'F'), n3 - (ans[3][pos] == 'F')); 
	}
	else if (n == 2) {
		res1 = DP_P(pos - 1, n1 - (ans[1][pos] == 'T'), n2 - (ans[2][pos] == 'T'), 0);
		res2 = DP_P(pos - 1, n1 - (ans[1][pos] == 'F'), n2 - (ans[2][pos] == 'F'), 0); 
	}
	else {
		res1 = DP_P(pos - 1, n1 - (ans[1][pos] == 'T'), 0, 0);
		res2 = DP_P(pos - 1, n1 - (ans[1][pos] == 'F'), 0, 0); 
	}
	return anss = res1 + res2;
}
char res[maxn];
...
int main(){ 
	...
	rep(TT, 1, T){
		...
		fact anss = fact(0, 1);
		rep(i, 1, m) {
			int id = ((ans[1][i] == 'T' ? 0 : 1) << 2) + ((ans[2][i] == 'T' ? 0 : 1) << 1) + ((ans[3][i] == 'T' ? 0 : 1) << 0);
			if (!hav[id]) {
				rep(p1, 0, p[1]) rep(p2, 0, p[2]) rep(p3, 0, p[3]) {
					if (n == 3) {
						num[id][0] += DP_P(i - 1, p1, p2, p3) * DP_S(i + 1, p[1] - p1 - (ans[1][i] == 'T'), p[2] - p2 - (ans[2][i] == 'T'), p[3] - p3 - (ans[3][i] == 'T'));
						num[id][1] += DP_P(i - 1, p1, p2, p3) * DP_S(i + 1, p[1] - p1 - (ans[1][i] == 'F'), p[2] - p2 - (ans[2][i] == 'F'), p[3] - p3 - (ans[3][i] == 'F'));
					}
					else if (n == 2) {
						num[id][0] += DP_P(i - 1, p1, p2, p3) * DP_S(i + 1, p[1] - p1 - (ans[1][i] == 'T'), p[2] - p2 - (ans[2][i] == 'T'), 0);
						num[id][1] += DP_P(i - 1, p1, p2, p3) * DP_S(i + 1, p[1] - p1 - (ans[1][i] == 'F'), p[2] - p2 - (ans[2][i] == 'F'), 0);
					}
					else {
						num[id][0] += DP_P(i - 1, p1, p2, p3) * DP_S(i + 1, p[1] - p1 - (ans[1][i] == 'T'), 0, 0);
						num[id][1] += DP_P(i - 1, p1, p2, p3) * DP_S(i + 1, p[1] - p1 - (ans[1][i] == 'F'), 0, 0);
					}
					
				} 
				hav[id] = 1;
			}
			anss = anss + fact(max(num[id][0], num[id][1]), num[id][0] + num[id][1]);
			if (num[id][0] > num[id][1]) {
				res[i] = 'T';
			}
			else {
				res[i] = 'F';
