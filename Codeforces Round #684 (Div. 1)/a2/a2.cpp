...
int main(){
	...
	while (T--){
		...
		//rep(i, 1, n) {{rep(j, 1, m) printf("%d", g[i][j]);} printf("\n");}
		rep(i, n, n) rep(j, 1, m) {
			if (g[i][j]) {
				if (j + 1 <= m && g[i][j + 1]) {
					ele op; op.a[0] = pii(i - 1, j), op.a[1] = pii(i - 1, j + 1), op.a[2] = pii(i, j), op.a[3] = pii(i, j + 1);
					op.type = 2, lis[++ans] = op;
					op.type = 3, lis[++ans] = op;
					g[i][j] ^= 1, g[i][j + 1] ^= 1;
					
				}
				else if (j + 2 <= m && g[i][j + 2]) {
					ele op; op.a[0] = pii(i - 1, j), op.a[1] = pii(i - 1, j + 1), op.a[2] = pii(i, j), op.a[3] = pii(i, j + 1);
					op.type = 0, lis[++ans] = op;
					op.a[0] = pii(i - 1, j + 1), op.a[1] = pii(i - 1, j + 2), op.a[2] = pii(i, j + 1), op.a[3] = pii(i, j + 2);
					op.type = 1, lis[++ans] = op;
					g[i][j] ^= 1, g[i][j + 2] ^= 1; 
				}
				else {
					if (j == m) operate(i - 1, j - 1, 3);
					else operate(i - 1, j, 2);
				}
			}
		}
