...
pii sta[maxn * maxn * 4];
int cs = 0, ct = 0;
void dfs(int x, int y, int di, int shit, int shit2 = 0){
    ...
    while (1){
    	if (shit){
		    ...
		    ct++;
		    if (shit2){
				id[x + maxn][y + maxn] = id[x + maxn][y + maxn + 1] = id[x + maxn - 1][y + maxn + 1] = 1;
		        sta[++cs] = pii(x, y), sta[++cs] = pii(x, y + 1), sta[++cs] = pii(x - 1, y + 1);
		        ct += 3;
		        printf("16"); 
		        //dfs(-2, 0, 4, 1);
		        shit2 = 0;
		    ...
		    if (is_up(x, y)){
		        ...
		        per(i, di + 5, di + 1){
		            if (i == di) continue;
		            int _x = x + dir[i % 6][0], _y = y + dir[i % 6][1];
		            if (ok(_x, _y) && dist(_x, _y) > d) {x = _x, y = _y, di = i % 6, shit2 = 1; goto ok;}
		        }
		        if (di != -1){
		            rep(i, di, di){ 
		                int _x = x + dir[i % 6][0], _y = y + dir[i % 6][1];
