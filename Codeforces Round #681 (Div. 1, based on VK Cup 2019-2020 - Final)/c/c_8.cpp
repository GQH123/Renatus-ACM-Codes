...
void Dij(int S){
	rep(i, 0, maxb) rep(j, 1, n) d[i][j] = pii(inf, inf);
	d[0][S] = pii(0, 0);
	pq.insert(ppp(d[0][S], pii(0, S)));
	while (!pq.empty()){
		pii x = pq.begin() -> se; pq.erase(pq.begin()); if (vis[x.fi][x.se]) continue;
		vis[x.fi][x.se] = 1;
		bool rev = x.fi & 1, add = 0;
		erep(i, x.se){ 
			int op = e[i].to;
			pii _d = d[x.fi][x.se];
			if ((i & 1) ^ rev) _d.fi++, add = 1;
			_d.se++;  
			int nxt = min(maxb, x.fi + add);
			if (vis[nxt][op]) continue;
			if (_d < d[nxt][op]) d[nxt][op] = _d, pq.insert(ppp(_d, pii(nxt, op)));
		}
	}
	dis ans = dis(d[0][n].fi, d[0][n].se);
	rep(i, 0, maxb) {
		ans = min(ans, dis(d[i][n].fi, d[i][n].se));
		/*
